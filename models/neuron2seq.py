...
class Neu2seq_test(nn.Module):
    def __init__(self, in_channels, base_channels, down_kernel_list, stride_list,
                 dim, encoder_depth, decoder_depth, heads, dropout, vocab_size, pad_idx):
        super(Neu2seq_test, self).__init__()
        assert len(down_kernel_list) == len(stride_list)
        self.downs = []
        self.pad_idx = pad_idx
    ...
    def forward(self, img, tgt):
        ...
        pos = posemb_sincos_3d(img)
        pos = pos.unsqueeze(0)
        src = img.flatten(2).permute(0, 2, 1)
        ...
        tgt = self.with_pos_embed(tgt_embedding, seq_pos)

        memory = self.encoder(src)
        hs = self.decoder(tgt, memory, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_padding_mask)
        out= self.class_head(hs)
...
if __name__ == '__main__':
    ...
    model = Neu2seq_test(in_channels=1, base_channels=24, down_kernel_list=args.down_kernel_list, 
